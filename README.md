...
[![nodesource/node](http://dockeri.co/image/gibby/puppeteer)](https://hub.docker.com/r/gibby/puppeteer/)
...
```bash
docker run --shm-size 1G --rm -v <path_to_script>:/app/index.js gibby/puppeteer:latest
```
...
```bash
docker run --shm-size 1G --sysctl net.ipv6.conf.all.disable_ipv6=1 --rm -v <path_to_script>:/app/index.js gibby/puppeteer:latest
```
